...
DIAGNOSTIC_BUILD = "pandastack-provider-v3"
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    async def execute(
        ...
        try:
            ...
            return _diagnostic_502(settings, error)
        return {
...
def _diagnostic_502(settings: Settings, error: Exception) -> JSONResponse:
    content: dict[str, object] = {
        "error": "search provider failed safely",
    }

    debug_enabled = (
        os.environ.get("SANDBOX_DEBUG_ERRORS", "").strip().lower()
        in {"1", "true", "yes"}
    )
    ...
    if debug_enabled:
        content["diagnostic"] = {
            ...
            "message": _redact_diagnostic(str(error), settings)[:2_000],
            "traceback": _redact_diagnostic(traceback.format_exc(), settings)[:10_000],
        }

    return JSONResponse(status_code=502, content=content)


def _redact_diagnostic(value: str, settings: Settings) -> str:
    redacted = value
    for secret in (settings.sandbox_token, settings.firecrawl_api_key):
        if secret:
